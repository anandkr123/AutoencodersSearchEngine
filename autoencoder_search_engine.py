...
# Building the encoder, used only during preparing encoded matrix as it doesn't contain any noise
def encoder(x,y):
    # Encoder Hidden layer with sigmoid activation #1
    layer_1 = tf.nn.sigmoid(tf.add(tf.matmul(x, weights['encoder_h1']),
                                   biases['encoder_b1']))
...
encoder_op1 = encoder1(X, nse)
decoder_op = decoder(encoder_op1)
